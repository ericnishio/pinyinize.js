...
(function() {
    ...
    /**
     * The global PinyinConverter object.
     */
    window.Pinyinize = {
        /**
         * Converts a string to pinyin.
         * @param {string} pinyinWithNumbers
                ...
                for (key in phrases) {
                    if (phrases.hasOwnProperty(key)) {
                        ...
                        if (termParts[1] !== '') { // if conversion was made // TODO: Improve this check.
                            pinyinWithMarks = pinyinWithMarks !== ''
                                ? pinyinWithMarks = pinyinWithMarks + ' ' + convertedTerm // append the term
                                : convertedTerm;
                        } else { // if no conversion was made
                            pinyinWithMarks = pinyinWithMarks !== ''
                                ? pinyinWithMarks + ' ' + value // append the term
                                : value;
                        }
                    }
                }
            } else {
                termParts = matchTerm(pinyinWithNumbers);
                convertedEnding = reconstructEnding(termParts[2], termParts[1]);
                convertedTerm = termParts[0] + convertedEnding;

                pinyinWithMarks = convertedTerm !== ''
                    ? convertedTerm
                    : pinyinWithNumbers;
            }
    ...
    function splitPhrase(phrase) {
        ...
        if (phrase.match(/\s/)) { // if string contains a space
            if (debug) console.log('Multiple terms detected.');
            return phrase.split(' ');
        } else {
            if (debug) console.log('One term detected.');
            return phrase;
        }
    }
    ...
    function matchTerm(term) {
        ...
        for (key in endings) {
            if (endings.hasOwnProperty(key)) {
                ...
                if (term.match(value)) {
                    if (debug) console.log('Term matched with ending \'' + key + '\'.');
    ...
    function reconstructEnding(ending, tone) {
        if (tone !== '' && ending !== '') {
            if (debug) console.log('Reconstructing ending \'' + ending + '\' with Tone ' + tone + '...');
